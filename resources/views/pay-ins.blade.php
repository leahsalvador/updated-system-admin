                ...
                <div class="order-last order-md-first">
                    <div class="card border-white card-home card-nav px-3">
                        <div class="card-header px-0 py-3">
                            <h5 class="fw-bold mb-0"><img class="me-2" src="{{ url('Icon%20awesome-money-bill-alt.png') }}">Pay-In</h5>
                        </div>
                        <div class="card-body px-0 py-2">
                            <p class="card-text mb-0" style="font-size: 14px;">Approved</p>
                            <h3 class="fw-bold text-success card-text">P {{ $approved }}</h3>
                            <p class="text-end card-text mb-0" style="font-size: 12px;">Pending</p>
                            <h5 class="fw-bold text-end text-danger card-text">P {{ $pending }}</h5>
                        </div>
                    </div>
                </div>
                <div>
            ...
            <div class="card-table px-4 py-3">
                ...
                <div class="d-flex flex-column flex-lg-row justify-content-lg-between custom-tabs-search pb-2">
                    <ul class="nav nav-tabs text-capitalize fs-5 fw-bolder custom-tabs" role="tablist">
                        ...
                        <li class="nav-item" role="presentation"><a class="nav-link active-approved py-1" role="tab" data-bs-toggle="tab" href="#tab-2">Approved</a></li>
                            ...
                            <table class="table table-borderless custom-table">
                                ...
                                @foreach($pendingData as $p)
                                    <tr>
                                        ...
                                        <td>{{ $p->value}}</td>
                                        <td>{{ $p->mop }}</td>
                                        <td><a href="{{ $p->receipt_image}}" class="btn btn-outline-light btn-sm text-nowrap" role="button">View Image</a></td>
                                        <td>Username</td>
                                        <td class="text-danger">Pending</td>
                                        <td class="text-nowrap">
                                            <form method="post" action="/fund/approved">
                                                {{ @csrf_field() }}
                                            <input type="hidden" value=" {{$p->id}}" name="id"/>
                                            <input type="submit" class="btn btn-success btn-sm text-nowrap me-2" role="button" value="Approve"/>
                                            </form>
                                            <form method="post" action="/fund/deleted">
                                                {{ @csrf_field()}}
                                                <input type="hidden" value=" {{$p->id}}" name="id"/>
                                            <input type="submit" class="btn btn-danger btn-sm text-nowrap" role="button" value="Delete">
                                            </form>
                                        </td>
                                        <td>—</td>
                                        <td>—</td>
                            ...
                            <table class="table table-borderless custom-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $a->value }}</td>
                                        <td>{{ $a->mop }}</td>
                                        <td><a class="btn btn-outline-light btn-sm text-nowrap" role="button">View Image</a></td>
                                        <td>Username</td>
